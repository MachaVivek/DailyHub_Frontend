...
const Fdish = () => {
    ...
    const submithandler = (e) => {
        ...
        try {
            ...
            if (f1 === 1) {
                axios.post(`${process.env.REACT_APP_BACKEND_URL}/postfdish`, {
                    email: email,
                    dname: dname,
                    why: why,
                });
    ...
    const gethandler = (e) => {
        e.preventDefault()
        axios.get(`${process.env.REACT_APP_BACKEND_URL}/getfdish/${emaill}`).then((res) => {
            let x = 0;
            ...
            for (let i = 0; i < res.data["data"].length; i++) {
                final.push({
                    dname: usingkeyde(res.data["data"][i]["dname"]),
    ...
    return (
        <div style={{border:"4px solid black", backgroundColor:"#650CFF",padding:"2%" }}>
            <h1 style={{textAlign:"center",color:"white"}}>Favorite Foods In Your Life</h1>

            <div style={{margin:"5px",padding:"10px",backgroundColor:"#650CFF"}}>
            <div style={{margin:"5px",border:"4px solid black",padding:"10px",backgroundColor:"white",borderRadius:"15px"}}>
                        ...
                        {currentCards.map((ind, index) => (
                            <div
                                key={index}
                                style={{margin:"2%",border:"4px solid black",padding:"1%",backgroundColor:"#650CFF",borderRadius:"15px",color:"white",textAlign:"left"}}
                            >
                                <br />
                                ...
                                <br />
                                Dish name: <div className="showtext">
                                    {ind.dname}
                                </div>
                                <br />
                    ...
                    <div style={{ margin: "5px", border: "4px solid black", padding: "10px", backgroundColor: "white", borderRadius: "15px", color: "white", textAlign: "left" }}>
                        <Form onSubmit={submithandler} >
                            ...
                            <InputGroup size="sm" className="mb-3">
                                <InputGroup.Text id="inputGroup-sizing-sm">Dish Name </InputGroup.Text>
                                <Form.Control
                                    aria-label="Small"
